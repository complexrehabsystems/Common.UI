...
namespace Common.UI.Components
{
    public class ObjectModel:StaticModel
    {
        ...
        public void LoadMesh(string modelPath)
        {
            var mesh = new Mesh();
        ...
        public void LoadTexturedMesh(string modelPath, string texturePath)
        {
            var mesh = new Mesh();
            ...
            var texture = Application.ResourceCache.GetTexture2D(texturePath);
            if (texture == null)
                return;
            ...
            var idata = mesh.GetTexturedIndexData(vdata);
